...
async function sendWelcome(sock, groupId, caption, mentions, pfp) {
  try {
    await sock.sendMessage(groupId, {
      image: { url: pfp },
      caption,
    ...
    console.log('⚠️ No se pudo enviar imagen welcome, enviando texto:', e?.message || e);
...
async function onParticipantsUpdate(sock, update) {
  ...
  console.log('👥 Acción:', action, '| Grupo:', id, '| Participantes:', participants.length);

  const enabled = await db.getGroupSetting(id, 'welcome');
  console.log('📌 Welcome activo:', enabled);
  ...
  try {
    ...
    console.log('⚠️ No se pudo obtener metadata:', e?.message || e);
  }
...
╰──────────────`;
      ...
      await sendWelcome(sock, id, caption, [user], pfp);
    }
