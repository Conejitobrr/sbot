...
const DEFAULT_PFP = 'https://i.imgur.com/JP4QZ7G.jpeg';
...
// ═══════════════════════════════════════
// FOTO PERFIL CON CACHE + FALLBACK ROBUSTO
// ═══════════════════════════════════════
async function getProfilePictureWithRetry(sock, user, retries = 3) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      // Si WhatsApp rate-limitea, usar cache previa o fallback
      if (status === 429) {
        if (cached?.url) return cached.url;
        return DEFAULT_PFP;
      }

      // Si no tiene foto / cualquier otro error
      if (i === retries - 1) {
        profileCache.set(user, {
          url: DEFAULT_PFP,
          expires: Date.now() + PROFILE_CACHE_TTL
