...
function getDefaultPfpBuffer() {
  try { if (fs.existsSync(DEFAULT_PFP)) return fs.readFileSync(DEFAULT_PFP); } catch {}
  return null;
}
...
function init(sock) {
  ...
  sock.ev.on('group-participants.update', async update => {
    ...
    catch (e) { console.log('❌ Error en welcome/goodbye:', e?.message || e); }
  });
...
async function maybeStartEvent(sock, groupId = null) {
  ...
  if (groupId) await announceEvent(sock, state.active);
...
async function onMessage(ctx) {
  ...
  // 🔥 APAGADO ABSOLUTO: Si el bot está desactivado en el grupo, se ignoran los eventos
  ...
  if (state.active.type === 'bonus' && text === 'yo' && !state.active.winner) {
    ...
    if (p) {
      gain *= 2; 
      const video = getPetVideo(p.type, 'celebrando', p.level);
      const txt = `🎉 ¡Qué velocidad! ${pushName} ganó *+${gain} XP*\n\n✨ ¡Gracias a la ayuda de tu mascota *${p.name}*, obtuviste el DOBLE de recompensa!`;
      if (video) msgContent = { video, caption: txt, gifPlayback: true };
      else msgContent = { text: txt };
    } else {
      msgContent = { text: `🎉 ¡Qué velocidad! ${pushName} ganó *+${gain} XP*` };
    }

    await db.addXP(sender, gain);
    await sock.sendMessage(remoteJid, msgContent);
  ...
  if (state.active.type === 'rob' && text === 'robaxp' && !state.active.winner) {
    ...
    if (p) {
      bonus *= 2; 
      const video = getPetVideo(p.type, 'celebrando', p.level);
      const txt = `😈 Misión cumplida. ${pushName} se robó el botín de *+${bonus} XP*\n\n✨ ¡Tu mascota *${p.name}* te ayudó a cargar el botín! Obtienes el DOBLE de experiencia.`;
      if (video) msgContent = { video, caption: txt, gifPlayback: true };
      else msgContent = { text: txt };
    } else {
      msgContent = { text: `😈 Misión cumplida. ${pushName} se robó el botín de *+${bonus} XP*` };
    }
  ...
  if (state.active.type === 'trivia' && text === state.active.answer && !state.active.winner) {
    ...
    if (p) {
      gain *= 2; 
      const video = getPetVideo(p.type, 'celebrando', p.level);
      const txt = `🏆 ¡Correcto! ${pushName} acertó y se lleva *+${gain} XP*\n\n✨ ¡*${p.name}* aúlla de felicidad por tu victoria! Obtienes el DOBLE de experiencia.`;
      if (video) msgContent = { video, caption: txt, gifPlayback: true };
      else msgContent = { text: txt };
    } else {
      msgContent = { text: `🏆 ¡Correcto! ${pushName} acertó y se lleva *+${gain} XP*` };
    }
  ...
  if (state.active.type === 'double' && text === 'doble' && !state.active.winner) {
    ...
    if (p) {
      const video = getPetVideo(p.type, 'celebrando', p.level);
      const txt = `⚡ ${pushName} ha reclamado la energía. ¡Todos ganan *doble XP* durante este evento!\n\n✨ ¡A *${p.name}* le encanta esta energía!`;
      if (video) msgContent = { video, caption: txt, gifPlayback: true };
      else msgContent = { text: txt };
    } else {
      msgContent = { text: `⚡ ${pushName} ha reclamado la energía. ¡Todos ganan *doble XP* durante este evento!` };
    }
...
async function getProfilePictureWithRetry(sock, user, retries = 2) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (i === retries - 1) {
        const data = { buffer: getDefaultPfpBuffer(), expires: Date.now() + PROFILE_CACHE_TTL };
        profileCache.set(user, data);
        return data;
      }
  ...
  return { buffer: getDefaultPfpBuffer(), expires: Date.now() + PROFILE_CACHE_TTL };
}
...
async function onParticipantsUpdate(sock, update) {
  ...
  // 🔥 APAGADO ABSOLUTO: Tampoco se envían bienvenidas si el bot entero está apagado
