...
// 🔥 CONFIGURACIÓN AJUSTADA: Más tiempo y más mensajes requeridos
const CFG = {
  minInterval: 30 * 60 * 1000, // Mínimo 30 minutos entre eventos (antes 10 min)
  maxInterval: 60 * 60 * 1000, // Máximo 60 minutos entre eventos (antes 20 min)
  duration: 60 * 1000,         // El evento sigue durando 60 segundos
  activityThreshold: 100       // Requiere al menos 100 mensajes en el grupo (antes 25)
};
...
async function onMessage(ctx) {
  ...
  if (state.active.type === 'bonus' && text === 'yo' && !state.active.winner) {
    ...
    const gain = rand(1500, 3000); // Antes era 25-60
  ...
  if (state.active.type === 'rob' && text === 'robaxp' && !state.active.winner) {
    ...
    const bonus = rand(1000, 2500); // Antes era 10-30
  ...
  if (state.active.type === 'trivia' && text === state.active.answer && !state.active.winner) {
    ...
    const gain = rand(2000, 4000); // Antes era 40-80
    await db.addXP(sender, gain);
