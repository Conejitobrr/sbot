...
async function finishDuel(sock, remoteJid, challenger, opponent, bet = 0) {
  ...
  for (let round = 1; round <= 8 && hp1 > 0 && hp2 > 0; round++) {
    ...
    if (dodge) {
      log.push(`🛡️ ${defenderName} ${pick(dodges)}.`);
      turn = defender;
      continue;
    }
    ...
    if (critical) {
      damage *= 2;
      log.push(`${pick(crits)}\n💥 ${attackerName} ${pick(attacks)}.`);
    } else {
      log.push(`⚔️ ${attackerName} ${pick(attacks)}.`);
    }

    if (defender === challenger) hp1 -= damage;
    else hp2 -= damage;

    log.push(`🩸 ${defenderName} perdió *${damage} HP*.`);
...
🥷 ${p1}
${progressBar(Math.max(0, hp1))} *${Math.max(0, hp1)} HP*

🗡️ ${p2}
${progressBar(Math.max(0, hp2))} *${Math.max(0, hp2)} HP*
...
module.exports = {
  ...
  async execute(ctx) {
    const { sock, msg, remoteJid, sender, args, command, fromGroup, isOwner } = ctx;
